...
BHAJAN_DB_FILE = os.path.join(SCRIPT_DIR, "Bhajans.xlsx")
...
@st.cache_data
def load_bhajan_data_from_excel(excel_file_path):
    """Load bhajan data from Excel file"""
    
    if not Path(excel_file_path).exists():
        st.error(f"❌ Bhajan database file not found: {excel_file_path}")
        st.info("Please make sure 'Bhajans.xlsx' is in the same folder as app.py")
        return None
    ...
    try:
        ...
        st.error(f"❌ Error reading Bhajans.xlsx: {e}")
        st.info("Please check that the Excel file has the correct columns: Category, Bhajan_Title, Author, Verse_Number, Original, English")
        return None

def get_default_bhajan_data():
    """This function is no longer used - Bhajans.xlsx is required"""
    return None
...
# === BHAJAN SEARCH PAGE ===
def bhajan_search_page():
    # Initialize session state for bhajan app
    if 'bhajan_page' not in st.session_state:
        st.session_state.bhajan_page = 'home'
    ...
    def show_titles():
        st.session_state.bhajan_page = 'titles'

    def show_categories():
        st.session_state.bhajan_page = 'categories'

    def show_authors():
        st.session_state.bhajan_page = 'authors'

    def show_category_bhajans(category):
        st.session_state.bhajan_page = 'category_bhajans'
        st.session_state.selected_category = category

    def show_author_bhajans(author):
        st.session_state.bhajan_page = 'author_bhajans'
        st.session_state.selected_author = author

    def show_bhajan(bhajan):
        st.session_state.bhajan_page = 'bhajan'
        st.session_state.selected_bhajan = bhajan

    # Load bhajan data
    bhajan_data = load_bhajan_data_from_excel(BHAJAN_DB_FILE)
    
    # Check if data loaded successfully
    if bhajan_data is None:
        st.error("❌ Cannot load Bhajan Search - Bhajans.xlsx file is required")
        st.stop()
    
    if len(bhajan_data) == 0:
        st.warning("⚠️ Bhajans.xlsx file is empty")
        st.stop()
    ...
    # Navigation
    if st.session_state.bhajan_page != 'home':
    ...
    elif st.session_state.bhajan_page == 'titles':
    ...
    elif st.session_state.bhajan_page == 'categories':
    ...
    elif st.session_state.bhajan_page == 'authors':
        st.markdown("## 👤 Authors")
    ...
    elif st.session_state.bhajan_page == 'category_bhajans':
    ...
    elif st.session_state.bhajan_page == 'author_bhajans':
    ...
    elif st.session_state.bhajan_page == 'bhajan':
...
def main():
    ...
    # Navigācijas izvēlne
    page = st.sidebar.radio("", ["Verse Finder", "Writings Database", "Bhajan Search"])
    
    if page == "Verse Finder":
        verse_finder_page()
    elif page == "Writings Database":
